(ns intercom.main
  "Kotodama WASM entrypoint for the Intercom clean-room actor (L4) — Clojure port.
  ...
  facts (`intercom.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "intercom")
(def tier "L4")
...
(def entity-specs
  [{:entity "Contact"      :plural "contacts"      :id-prefix "intercom_con"
    :fields [:email :phone :name :role :createdAt :signedUpAt] :required [:email :phone]
    :coerce {:signedUpAt :int}  :refs {}}
   {:entity "Conversation" :plural "conversations" :id-prefix "intercom_con"
    :fields [:state :priority :open :read :adminAssigneeId] :required [:state :priority]
    :coerce {:open :bool :read :bool :adminAssigneeId :int} :refs {}}
   {:entity "Company"      :plural "companies"     :id-prefix "intercom_com"
    :fields [:name :industry :monthlySpend :size :companyId] :required [:name :industry]
    :coerce {:monthlySpend :int :size :int}      :refs {:companyId "Company"}}
   {:entity "Admin"        :plural "admins"        :id-prefix "intercom_adm"
    :fields [:name :email :awayModeEnabled :hasInboxSeat :jobTitle] :required [:name :email]
    :coerce {:awayModeEnabled :bool :hasInboxSeat :bool} :refs {}}
   {:entity "Article"      :plural "articles"      :id-prefix "intercom_art"
    :fields [:title :body :state :authorId]      :required [:title :body]
    :coerce {:authorId :int}                     :refs {}}
   {:entity "Tag"          :plural "tags"          :id-prefix "intercom_tag"
    :fields [:name :appliedAt]                   :required [:name :appliedAt]
    :coerce {:appliedAt :int}                    :refs {}}])
...
(defn emit-facts
  "EAVT facts for one record: {\"intercom.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "intercom-compat" :tier tier :entities entities} 200])
...
(defn start! [] :intercom-compat/ready)
